...
思路：判断是否合法需要判断数独的每一行、每一列、9个小的九宫格中是否存在重复的数字。若每种情况均循环判断，则需要对整个二维数组遍历三次，还可以对此再优化；整个二维数组遍历一遍，对三种情况在循环中单独判断，而行和列的判断的判断较为简单，9个小的九宫格的遍历需要考虑下，对于(i/3*3+j/3)和(i%3*3+j%3)这两个编写需要思考下，比较巧妙的数学方法！
'''
