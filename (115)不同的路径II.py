...
思路：和“不同的路径”类似，也是一个斐波纳契数列问题，但当存在障碍物时，路径数是为0的；这里我们采用循环的方式，先创建一个和路径数据一样的二维数组result[][]来记录由起点出发到当前结点的路径数，不一样的是当路径数组的值为1时(既有障碍物时)，我们将对应位置的result置为0,表明没有路径可达（需要设定初始行和列的值），循环获得这个数据的路径数，返回要求位置的值即可。
'''
